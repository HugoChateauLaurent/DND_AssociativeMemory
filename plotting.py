...
def N_runs_capacity_graphs(N_runs, Ns, imgs, beta,fs,fn_labels, image_perturb_fn = halve_continuous_img, sep_fn = separation_max, sep_param = 1000, sigma=0.1,sname = "tiny_N_capacity_results.npy", figname = "tiny_N_runs_capacity_graph.jpg", load_data = False, plot_results = True, save_continuously=True,normalize_error_threshold = False):
    ...
    if plot_results:
        ...
        plt.title(dataset + " Similarity Functions")
        for i in range(len(fs)):
        ...
        plt.legend()
        fig.tight_layout()
        plt.savefig("figures/"+figname, format=SAVE_FORMAT)
        plt.show()
    return N_corrects
...
def N_runs_error_threshold_graphs(N_runs, error_thresholds, N_images, imgs, beta, fs, fn_labels, image_perturb_fn = halve_continuous_img, sep_fn = separation_softmax, sep_param=10, sigma=0.1, sname="cifar10_error_threshold_sweep.npy", figname="cifar10_error_threshold_sweep.jpg", load_data = False, plot_results = True, save_continuously=True):
    ...
    if plot_results:
        ...
        plt.title("Correct Retrievals by Error Threshold")
        for i in range(len(fs)):
        ...
        fig, ax = plt.gcf(), plt.gca()
        ax.spines[['right', 'top']].set_visible(False)
        plt.axvline(50, color="green", linestyle="--",label="Threshold Value")
...
def plot_separation_function_graph(Ns, imgs, beta,sep_fns, labels, image_perturb_fn = halve_continuous_img,sigma=1,f=manhattan_distance,use_norm = True,sep_param = 1000, plot_results = False):
...
def N_runs_separation_function_graphs(N_runs, Ns, imgs, beta,sep_fns,fn_labels, f = euclidean_distance, sep_fn = separation_max, sep_param = 1000, load_data = False,sname = "tiny_N_runs_separation_function_results_2.npy", figname = "tiny_N_runs_separation_functions_2.jpg", plot_results = True):
    if not load_data:
        N_corrects = []
        max_N = Ns[-1]
        for n in range(N_runs):
    ...
    if plot_results:
        ...
        # begin plot
        dataset = parse_sname_for_title(sname)
        plt.title(dataset + " Separation Functions")
...
def save_reconstructions_thresholds(imgs, N, f=manhattan_distance, image_perturb_fn = gaussian_perturb_image, sep_fn = separation_softmax,sep_param=8000, use_norm = True, sname="example_reconstructions_thresholds_saved_3"):
    X = imgs[0:N,:]
    img_shape = X[0].shape
...
def generate_demonstration_reconstructions(imgs, N, f=manhattan_distance, image_perturb_fn = mask_continuous_img, perturb_vals =[], sep_fn = separation_max, sep_param=1000, use_norm=True,sname=""):
    X = imgs[0:N,:]
    img_shape = X[0].shape
    img_len = np.prod(np.array(img_shape))
    if len(img_shape) != 1:
...
def visualize_heteroassociative_MCHN(imgs, N,N_imgs=5, f=manhattan_distance, sep_fn = separation_softmax, sep_param =100000, use_norm=True):
    X = imgs[0:N,:]
    img_shape = X[0].shape
    img_len = np.prod(np.array(img_shape))
...
def N_runs_auto_vs_heteroassociative(N_runs, Ns, imgs, beta,f = normalized_dot_product, image_perturb_fn = halve_continuous_img, sep_fn = separation_identity, sep_param = 1000, sigma=0.1,sname = "tiny_N_capacity_results.npy", figname = "tiny_N_runs_capacity_graph.jpg", load_data = False, plot_results = True, save_continuously=True, network_type=""):
    ...
    if plot_results:
        ...
        plt.title("Memory Capacity for Auto vs. Heteroassociative")
        # auto
...
def run_separation_function_experiments(imgs, dataset_str, ):
    ...
    sep_fns = [separation_identity, separation_softmax, separation_10max, separation_5max, separation_2max, separation_max]
    sep_labels = ["Identity", "Softmax","Norm. 10-Max","Norm. 5-Max","Norm. 2-Max","Max"]
    # do with fewer labels just to see if it works
    
    #Ns = [2,3,5,7,10,20,50,100,200,300]
    # Ns = [5,7,10,20,50,100,200,300,500,700,1000]
    Ns = [10,20,50,100,200,300,500,700,1000] # avoids problem with 10 Max
    Ns = [10,300,500]
    beta = 1
    N_runs = 5
...
def run_noise_levels_experiments(imgs, dataset_str):
    ...
    N = 100
    N_runs = 5
    fs = [euclidean_distance, manhattan_distance,normalized_dot_product,KL_divergence,reverse_KL_divergence,Jensen_Shannon_divergence]
...
def run_similarity_function_experiments(imgs, dataset_str,normalize_error_threshold=False):
    ...
    N_runs = 5
    beta = 1000
    #fs = [euclidean_distance, manhattan_distance,normalized_dot_product]#,KL_divergence,reverse_KL_divergence,Jensen_Shannon_divergence]#,cosine_similarity]
    fs = [euclidean_distance, manhattan_distance,normalized_dot_product,KL_divergence,reverse_KL_divergence,Jensen_Shannon_divergence]#,cosine_similarity]
    labels = ["Euclidean Distance","manhattan Distance", "Dot Product","KL Divergence","Reverse KL","Jensen-Shannon"]
...
def run_error_threshold_sweep(imgs, dataset_str):
    ...
    N_runs = 5
    beta = 1
    fs = [euclidean_distance, manhattan_distance,normalized_dot_product]#,cosine_similarity]
...
def run_additional_perturbation_experiments(imgs, dataset_str):
    ...
    N = 300
    N_runs =5
    fs = [euclidean_distance, manhattan_distance,normalized_dot_product]
    labels = ["Euclidean Distance","Manhattan Distance", "Dot Product"]
    beta = 1
    #random mask frac
...
if __name__ == '__main__':
    ...
    for dataset_str in ["mnist_", "cifar_10_", "tiny_"]:
        ...
        # # visualize heteroassicative (Fig. 5?)
        # run_visualize_heteroassociative(imgs, dataset_str)
        # # heteroassociation experiments (Fig. 6)
